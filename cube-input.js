...
function collectFaces() {
  ...
  let allFilled = true;
  FACE_META.forEach(f => {
    out[f.key] = faceData[f.key].map((v) => {
...
function validateScramble(str) {
  ...
  for (const p of parts) {
    if (!isValidMove(p)) return { valid: false, error: `"${p}" is not a valid move.` };
  }
  return { valid: true };
}
...
function renderSolution(result, container) {
  if (!result || !result.steps) {
    container.innerHTML = '<p class="solve-error">Could not compute solution.</p>';
    return;
  }

  if (result.steps.length === 0 || !result.steps[0].moves) {
    container.innerHTML = `
  ...
  let html = `
    <div class="solve-summary">
      <div class="solve-stat"><span class="solve-stat-n">${result.steps.length}</span><span>stages</span></div>
      <div class="solve-stat"><span class="solve-stat-n">${totalMoves}</span><span>moves</span></div>
      <div class="solve-status ok">Fully solved</div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-random-scramble')?.addEventListener('click', () => {
    ...
    if (inputEl) inputEl.value = scramble.join(' ');
  });
